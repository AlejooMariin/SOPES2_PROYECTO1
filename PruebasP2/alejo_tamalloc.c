...
SYSCALL_DEFINE1(alejo_tamalloc, size_t, size) {
    unsigned long addr;
    size_t aligned_size;
    ...
    if (size == 0) {
        printk(KERN_ERR "alejo_tamalloc: Tamaño de memoria no puede ser 0.\n");
    ...
    if (aligned_size == 0) {
        printk(KERN_ERR "alejo_tamalloc: Error en la alineación del tamaño.\n");
    ...
    if (IS_ERR_VALUE(addr)) {
        printk(KERN_ERR "alejo_tamalloc: Error al mapear la memoria virtual.\n");
        return addr; // Retornar el código de error
    }

    // Mensaje de éxito
    printk(KERN_INFO "alejo_tamalloc: Memoria asignada exitosamente en 0x%lx, tamaño: %zu bytes\n", addr, aligned_size);
