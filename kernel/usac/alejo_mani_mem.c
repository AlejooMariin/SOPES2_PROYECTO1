...
SYSCALL_DEFINE2(alejo_add_memory_limit, pid_t, process_pid, size_t, memory_limit) {
    ...
    if (process_pid < 0 || memory_limit <= 0) {
        pr_info("alejo_add_memory_limit: PID inválido (%d) o límite inválido (%zu).\n", process_pid, memory_limit);
        return -EINVAL;
    }
    ...
    if (!task) {
        pr_info("alejo_add_memory_limit: Proceso no encontrado (PID=%d).\n", process_pid);
    ...
    list_for_each_entry(temp, &memory_limit_list, list) {
        if (temp->pid == process_pid) {
            pr_info("alejo_add_memory_limit: PID %d ya está en la lista.\n", process_pid);
    ...
    if (!entry) {
        pr_info("alejo_add_memory_limit: Sin memoria para crear nodo (PID=%d).\n", process_pid);
        mutex_unlock(&memory_limit_mutex);
        return -ENOMEM;
    }
    ...
    pr_info("alejo_add_memory_limit: Nodo agregado (PID=%d, Limit=%zu bytes).\n", process_pid, memory_limit);
    ...
    if (security_task_setrlimit(task, RLIMIT_AS, &rl)) {
        pr_info("alejo_add_memory_limit: Fallo al aplicar RLIMIT_AS (PID=%d).\n", process_pid);
        kfree(entry);
        list_del(&entry->list);
    ...
    pr_info("alejo_add_memory_limit: RLIMIT_AS aplicado correctamente (PID=%d, Limit=%zu bytes).\n", process_pid, memory_limit);
...
SYSCALL_DEFINE3(alejo_get_memory_limits, struct memory_limitation __user *, u_processes_buffer, size_t, max_entries, int __user *, processes_returned) {
    ...
    kfree(k_buffer);
    pr_info("alejo_get_memory_limits: Nodo %d - PID=%d, Limit=%zu\n", count, temp->pid, temp->memory_limit);
    return 0;
}
...
SYSCALL_DEFINE2(alejo_update_memory_limit, pid_t, process_pid, size_t, memory_limit) {
    ...
    // Validar parámetros
    if (process_pid < 0 || memory_limit <= 0) {
        pr_info("Syscall alejo_update_memory_limit called1: PID=%d, New Limit=%zu bytes\n", process_pid, memory_limit);
        return -EINVAL;
    }
    ...
    mutex_lock(&memory_limit_mutex);
    pr_info("Syscall alejo_update_memory_limit called2: PID=%d, New Limit=%zu bytes\n", process_pid, memory_limit);
    ...
    list_for_each_entry(temp, &memory_limit_list, list) {
        if (temp->pid == process_pid) {
            ...
            put_task_struct(task);
            mutex_unlock(&memory_limit_mutex);
            pr_info("Syscall alejo_update_memory_limit called: PID=%d, New Limit=%zu bytes\n", process_pid, memory_limit);
...
SYSCALL_DEFINE1(alejo_remove_memory_limit, pid_t, process_pid) {
    ...
    // Validar PID
    if (process_pid <= 0) {
        pr_info("alejo_remove_memory_limit1: PID inválido (%d).\n", process_pid);
        return -EINVAL;
    }
    ...
    list_for_each_entry(temp, &memory_limit_list, list) {
        if (temp->pid == process_pid) {
            ...
            kfree(temp);
            mutex_unlock(&memory_limit_mutex);
            pr_info("alejo_remove_memory_limit2: PID %d eliminado correctamente.\n", process_pid);
            return 0;
        }
    }
    ...
    // Nodo no encontrado
    mutex_unlock(&memory_limit_mutex);
    pr_info("alejo_remove_memory_limit3: PID %d no encontrado en la lista.\n", process_pid);
